// This kernel we will do vectorized memory access
...
__global__ void finegemm_5_kernel(int M, int K, int N, const at::Half *a,
        ...
        for (int a_block_y_step = 0; a_block_y_step < BM;
            ...
            if (y_to_load < BM) {
                if (y_to_load + a_block_y < M) {
                    const float4 tmp = *reinterpret_cast<const float4 *>(a_block + y_to_load * K + x_to_load);
                    const half2 tmp_x = *reinterpret_cast<const half2 *>(&tmp.x);
                    const half2 tmp_y = *reinterpret_cast<const half2 *>(&tmp.y);
                    const half2 tmp_z = *reinterpret_cast<const half2 *>(&tmp.z);
                    const half2 tmp_w = *reinterpret_cast<const half2 *>(&tmp.w);
                    a_shared[x_to_load][y_to_load] = *reinterpret_cast<const half *>(&tmp_x.x);
                    a_shared[x_to_load + 1][y_to_load] = *reinterpret_cast<const half *>(&tmp_x.y);
                    a_shared[x_to_load + 2][y_to_load] = *reinterpret_cast<const half *>(&tmp_y.x);
                    a_shared[x_to_load + 3][y_to_load] = *reinterpret_cast<const half *>(&tmp_y.y);
                    a_shared[x_to_load + 4][y_to_load] = *reinterpret_cast<const half *>(&tmp_z.x);
                    a_shared[x_to_load + 5][y_to_load] = *reinterpret_cast<const half *>(&tmp_z.y);
                    a_shared[x_to_load + 6][y_to_load] = *reinterpret_cast<const half *>(&tmp_w.x);
                    a_shared[x_to_load + 7][y_to_load] = *reinterpret_cast<const half *>(&tmp_w.y);
                } else {
        ...
        for (int b_block_y_step = 0; b_block_y_step < BK;
            ...
            if (x_to_load < BN && y_to_load < BK) {
                if (x_to_load + b_block_x < N) {
                    const float4 tmp = *reinterpret_cast<const float4 *>(b_block + y_to_load * N + x_to_load);
                    const half2 tmp_x = *reinterpret_cast<const half2 *>(&tmp.x);
                    const half2 tmp_y = *reinterpret_cast<const half2 *>(&tmp.y);
                    const half2 tmp_z = *reinterpret_cast<const half2 *>(&tmp.z);
                    const half2 tmp_w = *reinterpret_cast<const half2 *>(&tmp.w);
                    b_shared[y_to_load][x_to_load] = *reinterpret_cast<const half *>(&tmp_x.x);
                    b_shared[y_to_load][x_to_load + 1] = *reinterpret_cast<const half *>(&tmp_x.y);
                    b_shared[y_to_load][x_to_load + 2] = *reinterpret_cast<const half *>(&tmp_y.x);
                    b_shared[y_to_load][x_to_load + 3] = *reinterpret_cast<const half *>(&tmp_y.y);
                    b_shared[y_to_load][x_to_load + 4] = *reinterpret_cast<const half *>(&tmp_z.x);
                    b_shared[y_to_load][x_to_load + 5] = *reinterpret_cast<const half *>(&tmp_z.y);
                    b_shared[y_to_load][x_to_load + 6] = *reinterpret_cast<const half *>(&tmp_w.x);
                    b_shared[y_to_load][x_to_load + 7] = *reinterpret_cast<const half *>(&tmp_w.y);
...
at::Tensor finegemm_5(const at::Tensor &a, const at::Tensor &b) {
    long M = a.size(0);
    long K = a.size(1);
    long N = b.size(1);
    ...
    finegemm_5_kernel<BM, BN, BK, 8, 8>
...
TORCH_LIBRARY_IMPL(gemm_unified, CUDA, m) { m.impl("finegemm_5", &finegemm_5); }
