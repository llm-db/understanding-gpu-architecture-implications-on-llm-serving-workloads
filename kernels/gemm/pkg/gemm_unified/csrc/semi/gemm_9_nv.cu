...
__global__ void finegemm_9_kernel(int M, int K, int N, const at::Half *a,
...
using namespace nvcuda;
    ...
    __shared__ half a_smem[(BK+8) * BM];
    __shared__ half b_smem[(BN+8) * BK];
    ...
    for (int k_idx = 0; k_idx < (K / BK); k_idx++) {
        ...
        const half *b_k = b_ptr + b_k_y * N + b_k_x;
        {
            ...
            for (int i = 0; (i + a_k_y_step) <= BM; i += a_k_y_step) {
                ...
                cooperative_groups::memcpy_async(group, a_smem + (i + y_idx) * (BK + 8) + (x_idx * 8), a_k + (i + y_idx) * K + (x_idx * 8), sizeof(half) * 8);
            ...
            for (int i = 0; (i + b_k_y_step) <= BK; i += b_k_y_step) {
                ...
                cooperative_groups::memcpy_async(group, b_smem + (i + y_idx) * (BN + 8) + (x_idx * 8), b_k + (i + y_idx) * N + (x_idx * 8), sizeof(half) * 8);
        ...
        __syncthreads();
        {
            ...
            for (int b_idx = 0; b_idx < (WN / 16); b_idx++) {
                const half* b_load_base = b_smem + (warp_b_y_base) * (BN + 8) + (warp_b_x_base + b_idx * 16);
            ...
            for (int a_idx = 0; a_idx < (WM / 16); a_idx++) {
                const half* a_load_base = a_smem + (warp_a_y_base + a_idx * 16) * (BK + 8) + (warp_a_x_base);
...
at::Tensor finegemm_9(const at::Tensor &a, const at::Tensor &b) {
    long M = a.size(0);
    long K = a.size(1);
    long N = b.size(1);
    ...
    finegemm_9_kernel<BM, BN, BK, 64, 64>
        <<<grid, block>>>(M, K, N, a_ptr, b_ptr, result_ptr);
    return result;
}

// Registers CUDA implementations for mymuladd, mymul, myadd_out
TORCH_LIBRARY_IMPL(gemm_unified, CUDA, m) { m.impl("finegemm_9", &finegemm_9); }
