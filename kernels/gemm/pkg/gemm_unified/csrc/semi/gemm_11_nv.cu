...
using namespace nvcuda;
    ...
    __shared__ half a_smem[3 * (BK) * BM];
    __shared__ half b_smem[3 * (BN + 8) * BK];
    constexpr int a_shared_mem_buff_size = (BK) * BM;
    ...
    // pipeline.producer_acquire();
    {
            ...
            for (int i = 0; (i + a_k_y_step) <= BM; i += a_k_y_step) {
                ...
                __pipeline_memcpy_async(a_smem + (i + y_idx) * (BK) + (x_idx * 8), a_k + (i + y_idx) * K + (x_idx * 8), sizeof(half) * 8);
            }
...

    {
            ...
            for (int i = 0; (i + a_k_y_step) <= BM; i += a_k_y_step) {
                const int y_idx = id_in_block / (BK / 8);
                const int x_idx = id_in_block % (BK / 8);
                __pipeline_memcpy_async(a_smem + a_shared_mem_buff_size + (i + y_idx) * (BK) + (x_idx * 8), a_k + (i + y_idx) * K + (x_idx * 8), sizeof(half) * 8);
            }
            constexpr int b_k_y_step = 128 / (BN / 8);
            #pragma unroll
            for (int i = 0; (i + b_k_y_step) <= BK; i += b_k_y_step) {
                const int y_idx = id_in_block / (BN / 8);
                const int x_idx = id_in_block % (BN / 8);
                __pipeline_memcpy_async(b_smem + b_shared_mem_buff_size + (i + y_idx) * (BN + 8) + (x_idx * 8), b_k + (i + y_idx) * N + (x_idx * 8), sizeof(half) * 8);
            }
            __pipeline_commit();
    }
    ...
    for (int k_idx = 0; k_idx < (K / BK); k_idx++) {
        const int k_idx_stage = k_idx % 3;
        {
            // preload the next one
            int k_idx_next = k_idx + 2;
            
            // pipeline.producer_acquire();
            const int k_idx_next_stage = k_idx_next % 3;
            k_idx_next = min(k_idx_next, (K / BK) - 1);
            // if (k_idx_next >= (K / BK)) {
            //     k_idx_next = (K / BK) - 1;
            // }
            const int a_k_y = c_block_y;
            ...
            for (int i = 0; (i + a_k_y_step) <= BM; i += a_k_y_step) {
                const int y_idx = id_in_block / (BK / 8);
                const int x_idx = id_in_block % (BK / 8);
                __pipeline_memcpy_async(a_smem + k_idx_next_stage * a_shared_mem_buff_size + (i + y_idx) * (BK) + (x_idx * 8), a_k + (i + y_idx) * K + (x_idx * 8), sizeof(half) * 8);
            }
            constexpr int b_k_y_step = 128 / (BN / 8);
            #pragma unroll
            for (int i = 0; (i + b_k_y_step) <= BK; i += b_k_y_step) {
        ...
        __syncthreads();
        {
            ...
            for (int a_idx = 0; a_idx < (WM / 16); a_idx++) {
                const half* a_load_base = a_smem + k_idx_stage * a_shared_mem_buff_size + (warp_a_y_base + a_idx * 16) * (BK) + (warp_a_x_base);
...
at::Tensor finegemm_11(const at::Tensor &a, const at::Tensor &b) {
    ...
    finegemm_11_kernel<BM, BN, BK, 64, 64>
        <<<grid, block>>>(M, K, N, a_ptr, b_ptr, result_ptr);
    return result;
}

// Registers CUDA implementations for mymuladd, mymul, myadd_out
TORCH_LIBRARY_IMPL(gemm_unified, CUDA, m) { m.impl("finegemm_11", &finegemm_11); }
