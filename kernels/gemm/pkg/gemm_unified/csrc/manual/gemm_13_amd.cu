...
__global__ void __launch_bounds__(256)
    finegemm_13_kernel(uint32_t m, uint32_t n, uint32_t k, at::Half const *A,
    ...
    half b_tmp[8];
    {
        ...
        int k_stage = k % 2;
        {
            const int a_dram_x = c_dram_x + (tid / 2);
            const int a_dram_y = k * 16 + 8 * (tid % 2);
            *reinterpret_cast<int4 *>(&a_tmp) = *reinterpret_cast<const int4 *>(a + (a_dram_x) * K + (a_dram_y));
        }
        {
            // load B into reg
            const int b_dram_x = c_dram_y + (tid / 2);
            const int b_dram_y = k * 16 + 8 * (tid % 2);
            *reinterpret_cast<int4 *>(&b_tmp) = *reinterpret_cast<const int4 *>(b + (b_dram_x) * K + b_dram_y);
        }
        {
            // write A into smem
        ...
        {
            // write B into smem
            const int b_smem_x = 128 + tid / 2;
            const int b_smem_y = 8 * (tid % 2);
            #pragma unroll
            for (int i = 0; i < 8; i++) {
    ...
    for(uint32_t currentK = ROCWMMA_K; currentK < k; currentK += ROCWMMA_K)
    {
        ...
        
        {
            ...
            k_next = min(k_next, (k / 16) - 1);
            {
                ...
                // }
                const int a_dram_x = c_dram_x + (tid / 2);
                const int a_dram_y = k_next * 16 + 8 * (tid % 2);
                *reinterpret_cast<int4 *>(&a_tmp) = *reinterpret_cast<const int4 *>(a + (a_dram_x) * K + (a_dram_y));
            }
            {
                // load B into reg
                ...
                // }
               const int b_dram_x = c_dram_y + (tid / 2);
                const int b_dram_y = k_next * 16 + 8 * (tid % 2);
                *reinterpret_cast<int4 *>(&b_tmp) = *reinterpret_cast<const int4 *>(b + (b_dram_x) * K + b_dram_y);
            }
        }
...
at::Tensor finegemm_13(const at::Tensor &a, const at::Tensor &b) {
    long M = a.size(0);
    long K = a.size(1);
    long N = b.size(1);
    ...
    finegemm_13_kernel<<<grid, block, ldsusage>>>(M, N, K, a_ptr, b_ptr, result_ptr);
...
TORCH_LIBRARY_IMPL(gemm_unified, CUDA, m) {
    m.impl("finegemm_13", &finegemm_13);
}
