...
if __name__ == "__main__":
    ...
    import pandas as pd
    df = pd.DataFrame(columns=["2^8", "2^9", "2^10", "2^11", "2^12", "2^13", "2^14"], index=["Kernel 0", "Kernel 2", "Kernel 4", "Kernel 5", "Kernel -2", "Kernel -1"])
    for i in [0, 2, 4, 5, -2, -1]:
        for j in range(8, 15):
            if i == 5:
                tflops = profile_finefmha_kernel(1, 32, 2**j, 128, True, True, False, i, dtype=torch.bfloat16)
            else:
    ...
    # profile_finefmha_kernel(1, 32, 2048, 128, True, True, False, 5, dtype=torch.bfloat16)
