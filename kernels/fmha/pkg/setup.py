...
if not SKIP_CUDA_BUILD and not IS_ROCM:
    ...
    ext_modules.append(
        CUDAExtension(
            name="fmha_unified._C",
            sources=[
                "fmha_unified/csrc/fmha_kernel.cpp",  # TODO: Add source files here
            ]
            + glob.glob("fmha_unified/csrc/auto/*.cu")
            + glob.glob("fmha_unified/csrc/semi/*_nv.cu")
            + glob.glob("fmha_unified/csrc/manual/*_nv.cu"),
            extra_compile_args={
            ...
            include_dirs=[
                os.path.abspath("../../deps/ThunderKittens/include")
            ],
        )
    )
    ...
    ext_modules.append(
        CUDAExtension(
            ...
            sources=rename_cpp_to_cu(
                [
                    "fmha_unified/csrc/fmha_kernel.cpp",
                ]
                + glob.glob("fmha_unified/csrc/auto/*.cu")
                + glob.glob("fmha_unified/csrc/semi/*_amd.cpp")
                + glob.glob("fmha_unified/csrc/manual/*_amd.cpp"),
            ),
            extra_compile_args={
