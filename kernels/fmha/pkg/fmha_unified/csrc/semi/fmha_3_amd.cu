...
__global__ void finefmha_3_kernel(const at::Half* Q, const at::Half* K,
    ...
    __shared__ half k_smem[4 * N_warp * 32 * 2];
    ...
    fragment<matrix_b, 16, 16, 16, half, row_major> V_frag[4];
    fragment<accumulator, 16, 16, 16, half> C_frag[2][4];
    ...
    if (tid < 64) {
        ...
        m_smem[tid] = -__float2half(INFINITY);
    }
    ...
    // here we preload the whole of Q
    #pragma unroll
    for (int i = 0; i < 4; i++) {
        const int smem_x = 16 * i + tid / 16;
        const int smem_y = 8 * (tid % 16);
        const int dram_x = smem_x;
        const int dram_y = smem_y;
        *reinterpret_cast<int4 *>(q_smem + (smem_x) * 128 + (smem_y)) = *reinterpret_cast<const int4 *>(q_block + (dram_x) * 128 + (dram_y));
    }

    int4 k_tmp;
    // start the preloading of a block of K, first to register, then to smem
    {
        const int j = 0;
        const int k = 0;
        int k_next = k + 1;
        const int k_next_stage = k_next % 2;
        k_next = min(k_next, (d / 32) - 1);
        {
            // load to register
            const int smem_x = tid / 4;
            const int smem_y = k_next_stage * 32 + 8 * (tid % 4);
            const int dram_x = j * 64 + smem_x;
            const int dram_y = k_next * 32 + 8 * (tid % 4);
            k_tmp = *reinterpret_cast<const int4 *>(k_block + (dram_x) * 128 + (dram_y));
        }

        {
            ...
            const int dram_x = j * 64 + smem_x;
            const int dram_y = k_next * 32 + 8 * (tid % 4);
            *reinterpret_cast<int4 *>(k_smem + (smem_x) * 64 + (smem_y)) = k_tmp;
        }
    }
    ...
    for (int j = 0; j < j_max; j++) {
        ...
        // 1.3 store s back to smem
        {
            ...
            for (int m = 0; m < 2; m++) {
                #pragma unroll
                for (int n = 0; n < 2; n++) {
                    fill_fragment(C_frag[m][n], __float2half(0.0));
                }
            }
            #pragma unroll
            for (int m = 0; m < 16; m++) {
        ...
        // start loop through k-dim of Q and K
        for (int k = 0; k < (d / 32); k++) {
            const int k_stage = k % 2;
            int k_next = k + 1;
            const int k_next_stage = k_next % 2;
            k_next = min(k_next, (d / 32) - 1);
            // 1. load current buffer to reg
            {
                const int warp_a_offset = 32 * warp_x;
                const int q_smem_x = warp_a_offset + id_in_warp % 32;
                #pragma unroll
                for (int m = 0; m < 4; m++) {
                    const int q_smem_y = k * 32 + m * 8 + 4 * (id_in_warp / 32);
                    zj_fragA[m] = *reinterpret_cast<halfx4 *>(q_smem + (q_smem_x) * 128 + (q_smem_y));
                }
                const int warp_b_offset = 32 * warp_y;
                const int k_smem_x = warp_b_offset + (id_in_warp % 32);
                for (int m = 0; m < 4; m++) {
                    const int k_smem_y = k_stage * 32 + m * 8 + 4 * (id_in_warp / 32);
                    zj_fragB[m] = *reinterpret_cast<halfx4 *>(k_smem + (k_smem_x) * 64 + (k_smem_y));
                }
            }
            // 2. load next stage to reg
            {
                // load to register
                const int smem_x = tid / 4;
                const int smem_y = k_next_stage * 32 + 8 * (tid % 4);
            ...
            // 3. mma of current reg
            for (int m = 0; m < 4; m++) {
                zj_fragAcc = __builtin_amdgcn_mfma_f32_32x32x8f16(zj_fragA[m], zj_fragB[m], zj_fragAcc, 0, 0, 0);
            }
            // 4. save next stage to smem
            {
                // write to smem
                const int smem_x = tid / 4;
                const int smem_y = k_next_stage * 32 + 8 * (tid % 4);
        ...
        // write back results
        const int warp_x_offset = warp_x * 32;
        const int warp_y_offset = warp_y * 32;
        const int frag16_x = 4 * (id_in_warp / 32);
        const int frag16_y = (id_in_warp % 32);
        for (int p = 0; p < 4; p++) {
            const int frag4_start_index = p * 4;
            const int frag4_x = frag16_x + p * 8;
            for (int q = 0; q < 4; q++) {
                const int smem_x = warp_x_offset + frag4_x + q;
                const int smem_y = warp_y_offset + frag16_y;
                s_p_smem[smem_x][smem_y] = __float2half(zj_fragAcc[frag4_start_index + q]) * (*reinterpret_cast<const half*>(&softmax_scale));
            }
        }
            ...
            if (j < j_max - 1) {
                ...
                for (int k = 0; k < 64; k++) {
                    ...
                    if (causal && q_in_seq < k_in_seq) {
                        s_p_smem[tid][k] = - __float2half(INFINITY);
        ...
        // 5.3 store o back to smem
        {
            ...
            for (int m = 0; m < 2; m++) {
                ...
                for (int n = 0; n < 4; n++) {
                    ...
                    load_matrix_sync(C_frag[m][n], &o_smem[o_frag_x][o_frag_y], 128, mem_row_major);
                }
            }
        }
        #pragma unroll
        for (int k = 0; k < (64/16); k++) {
            ...
            for (int m = 0; m < 2; m++) {
                ...
                const int sp_y = k * 16;
                load_matrix_sync(A_frag[m], &s_p_smem[sp_x][sp_y], 64);
            }
            #pragma unroll
            for (int n = 0; n < 4; n++) {
                const int v_x = 0;
                const int v_y = 64 * warp_y + 16 * n;
                load_matrix_sync(V_frag[n], &v_smem[v_x][v_y], 128);
            }
            #pragma unroll
            for (int m = 0; m < 2; m++) {
                #pragma unroll
                for (int n = 0; n < 4; n++) {
                    mma_sync(C_frag[m][n], A_frag[m], V_frag[n], C_frag[m][n]);
                }
            }
            __syncthreads();
        }
        ...
        for (int m = 0; m < 2; m++) {
            ...
            for (int n = 0; n < 4; n++) {
                const int o_frag_x = 32 * warp_x + 16 * m;
                const int o_frag_y = 64 * warp_y + 16 * n;
                store_matrix_sync(&o_smem[o_frag_x][o_frag_y], C_frag[m][n], 128, mem_row_major);
            }
        }
...
torch::Tensor finefmha_3(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
    ...
    dim3 block_dim(256);                  // Bc threads per block

    if (is_decode) {
        finefmha_3_kernel<64, 16, false, true, 128><<<grid_dim, block_dim>>>(
            Q.data_ptr<at::Half>(), K.data_ptr<at::Half>(),
            V.data_ptr<at::Half>(), H, H_kv, N_q, N_kv, softmax_scale,
            O.data_ptr<at::Half>(), decode_length);
    } else if (causal) {
        finefmha_3_kernel<64, 16, true, false, 128><<<grid_dim, block_dim>>>(
            Q.data_ptr<at::Half>(), K.data_ptr<at::Half>(),
            V.data_ptr<at::Half>(), H, H_kv, N_q, N_kv, softmax_scale,
            O.data_ptr<at::Half>(), decode_length);
    } else {
        finefmha_3_kernel<64, 16, false, false, 128><<<grid_dim, block_dim>>>(
            Q.data_ptr<at::Half>(), K.data_ptr<at::Half>(),
            V.data_ptr<at::Half>(), H, H_kv, N_q, N_kv, softmax_scale,
            O.data_ptr<at::Half>(), decode_length);
    }
...
TORCH_LIBRARY_IMPL(fmha_unified, CUDA, m) { m.impl("finefmha_3", &finefmha_3); }
